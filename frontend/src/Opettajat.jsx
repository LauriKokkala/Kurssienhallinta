...
function Opettajat() {
  ...
  const fetchData = () => {
    fetch("http://localhost:5000/list/opettajat")
  ...
  function sendData() {
    ...
    const data = { etunimi: name, sukunimi: surname, aine: subject}
    console.log(data)
    ...
    ? "http://127.0.0.1:5000/add/opettajat"
    : `http://127.0.0.1:5000/update/opettajat/${editId}`;
  ...
  function deleteClicked(id) {
    console.log("delete!")
    fetch("http://127.0.0.1:5000/delete/opettajat", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
            ...
            {info.map((row) => (
              ...
              editId === row.Tunnus ? (
                <Row
                  ...
                  data3={<input value={subject} onChange={(e) => setSubject(e.target.value)} type="text" />}
